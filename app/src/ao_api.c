...
static int ao_create_object(struct ao_t * ao, uint8_t * ao_data, uint8_t ao_data_size, ao_ev_handler_t ao_ev_f, ao_free_handler_t ao_free_f, ao_op_t ao_op)
{
	if(ao_data_size > AO_MAX_DATA_SIZE) return AO_E_SIZE;
	if(ao_data_size > 0 && ao_data == NULL) return AO_E_ARG;
	...
	if((ao_op & AO_OP_NO_QUEUE) != AO_OP_NO_QUEUE)
	{
		...
		if(ao->ao_queue == NULL) return AO_E_OS;
	}
	else
	{
		ao->ao_queue = NULL;
	...
	{
		BaseType_t rt = xTaskCreate(ao_task, "ao_task", 128, (void * const) ao, tskIDLE_PRIORITY + 1, &ao->ao_task);
		if(rt == pdFAIL)
		{
			if(ao->ao_queue != NULL)
			{
...
ao_t ao_init(uint8_t * ao_data, uint8_t ao_data_size, ao_ev_handler_t ao_ev_f, ao_free_handler_t ao_free_f, ao_op_t ao_op)
{
	for(uint8_t i = 0; i < AO_MAX_OBJECTS; i++)
	{
		if(false == ao_sys.ao_ins[i].used)
		{
			...
			int rt = ao_create_object(ao, ao_data, ao_data_size, ao_ev_f, ao_free_f, ao_op);
			if(rt != AO_OK) return NULL;
			return ao;
		}
	}
	return NULL;
...
uint8_t * ao_get_data(ao_t ao)
{
	...
	if(ao->ao_data_size == 0) return NULL;
	return ao->ao_data;
}
...
{
	if(!receiver) return AO_E_ARG; //Sender its optional
	if((receiver->ao_queue == NULL && !sender) && sender->ao_queue == NULL) return AO_E_SENDER; //If receiver does not use queue we must need a sender with queue in use.
	if(ao_msg == NULL) return AO_E_ARG;
	if(ao_msg_size == 0) return AO_E_ARG;
	if(ao_msg_size > 0 && ao_msg == NULL) return AO_E_SIZE;
	...
	QueueHandle_t hqueue = receiver->ao_queue == NULL? sender->ao_queue:receiver->ao_queue;

	BaseType_t rt = xQueueSend(hqueue, &ao_msg_o, 0);
	if(rt == pdFAIL) err = AO_E_OS;
...
{
	if(ao == NULL || ao->ao_free_f == NULL) return;
	ao->ao_free_f(ao_msg);
}
